...
public class DisplayListSystem implements Runnable {
    ...
    @WithFactory
    public DisplayListSystem(EntityEngine engine, @ByFactory(ScreenFactory.class) Screen screen) {
        this.engine = engine;
        fromRow = 0;
        toRow = fromRow + screen.getTerminalSize().getRows();
        fromCol = 0;
        toCol = fromCol + screen.getTerminalSize().getColumns();
    }
